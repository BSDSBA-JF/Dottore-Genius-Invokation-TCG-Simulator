...
class StartingHandSelectPhase(ph.Phase):
    ...
    def _activate(self, game_state: GameState) -> GameState:
        return game_state.factory().player1(
            game_state.player1.factory().phase(Act.ACTION_PHASE).build()
        ).player2(
            game_state.player2.factory().phase(Act.ACTION_PHASE).build()
        ).build()
    ...
    def _to_roll_phase(self, game_state: GameState) -> GameState:
        return game_state.factory().phase(
            ...
            game_state.player1.factory().phase(Act.PASSIVE_WAIT_PHASE).build()
        ).player2(
            game_state.player2.factory().phase(Act.PASSIVE_WAIT_PHASE).build()
        ).build()
    ...
    def step(self, game_state: GameState) -> GameState:
        ...
        elif game_state.effect_stack.is_not_empty():
    ...
    def _choices_helper(cls, action_generator: ActionGenerator) -> GivenChoiceType:
        ...
        return tuple(character.id for character in characters)
