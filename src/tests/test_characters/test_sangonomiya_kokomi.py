...
class TestSangonomiyaKokomi(unittest.TestCase):
    ...
    assert type(BASE_GAME.player1.just_get_active_character()) is SangonomiyaKokomi
    ...
    def test_normal_attack(self):
        game_state = step_skill(
            ...
            cost=ActualDice({Element.HYDRO: 1, Element.ELECTRO: 1, Element.DENDRO: 1}),
        )
    ...
    def test_elemental_burst(self):
        ...
        self.assertIn(Element.HYDRO, p2ac.elemental_aura)
        self.assertIn(CeremonialGarmentStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(CeremonialGarmentStatus).usages, 2)
    ...
    def test_bake_kurage_summon(self):
        ...
        self.assertIn(BakeKurageSummon, p1.summons)
        self.assertEqual(p1.summons.just_find(BakeKurageSummon).usages, 1)
        ...
        self.assertNotIn(BakeKurageSummon, p1.summons)
    ...
    def test_ceremonial_garment_status(self):
        ...
        self.assertIn(CeremonialGarmentStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(CeremonialGarmentStatus).usages, 2)
        self.assertEqual(p1cs.just_get_character(1).hp, 5)
        self.assertEqual(p1cs.just_get_character(2).hp, 6)
        self.assertEqual(p1cs.just_get_character(3).hp, 7)
        ...
        p1ac = game_state.player1.just_get_active_character()
        self.assertIn(CeremonialGarmentStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(CeremonialGarmentStatus).usages, 1)

        # usages decrease per round
        game_state = next_round(game_state)
        p1ac = game_state.player1.just_get_active_character()
        self.assertNotIn(CeremonialGarmentStatus, p1ac.character_statuses)
    ...
    def test_talent_card(self):
        ...
        self.assertEqual(p1ac.character_statuses.just_find(CeremonialGarmentStatus).usages, 2)
        self.assertIn(BakeKurageSummon, p1.summons)
        self.assertEqual(p1.summons.just_find(BakeKurageSummon).usages, 3)
