...
class TestJadeplumeTerrorshroom(unittest.TestCase):
    ...
    assert type(BASE_GAME.player1.just_get_active_character()) is JadeplumeTerrorshroom
    ...
    def test_normal_attack(self):
        game_state = step_skill(
            ...
            cost=ActualDice({Element.DENDRO: 1, Element.HYDRO: 1, Element.GEO: 1}),
        )
    ...
    def test_radical_vitality_status_usages(self):
        ...
        self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(RadicalVitalityStatus).usages, 0)
        ...
        p1ac = game_state.player1.just_get_active_character()
        self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(RadicalVitalityStatus).usages, 1)
        ...
        p1ac = game_state.player1.just_get_active_character()
        self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(RadicalVitalityStatus).usages, 2)

        # dealing elemental damage increases stacks
        game_state = step_skill(game_state, Pid.P1, CharacterSkill.SKILL2)
        p1ac = game_state.player1.just_get_active_character()
        self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(RadicalVitalityStatus).usages, 3)
    ...
    def test_radical_vitality_status_dmg_boost(self):
        for usages in range(0, 4):
            with self.subTest(usages=usages):
                ...
                p1ac = game_state.player1.just_get_active_character()
                p2ac = game_state.player2.just_get_active_character()
                self.assertEqual(p2ac.hp, 10 - (4 + usages))
                self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
                self.assertEqual(
                    p1ac.character_statuses.just_find(RadicalVitalityStatus).usages,
                    0
                )
    ...
    def test_radical_vitality_status_end_round(self):
        for usages in range(0, 4):
            with self.subTest(usages=usages):
                ...
                self.assertEqual(p1ac.energy, expected_energy)
                self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
                self.assertEqual(
                    p1ac.character_statuses.just_find(RadicalVitalityStatus).usages,
                    expected_stacks
    ...
    def test_talent_card(self):
        ...
        p1ac = game_state.player1.just_get_active_character()
        self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(RadicalVitalityStatus).usages, 4)
        game_state_4_stack = game_state

        # 3 stacks at end phase doesn't clear energy
        game_state = next_round(game_state_3_stack)
        p1ac = game_state.player1.just_get_active_character()
        self.assertEqual(p1ac.energy, 2)
        self.assertIn(RadicalVitalityStatus, p1ac.character_statuses)
        self.assertEqual(p1ac.character_statuses.just_find(RadicalVitalityStatus).usages, 3)
